...
const forgotPassword = (user) => {
  // console.log(user,"user====>");
  const query = `select * from registertable where email ='${user.email}';`;
  const result = db.promise().query(query);
  return result;
};
...
const insertNewCredentials = (user) => {
  ...
  const insertQuery = `UPDATE registertable SET password='${user.newPassword}' WHERE email = '${user.email}'`;
